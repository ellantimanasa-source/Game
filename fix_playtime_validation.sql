-- Fix the overly strict playtime validation
...
BEGIN
  ...
  -- Sanity check: score should be roughly achievable given play time
  v_base_score :=
  ...
  IF p_score > v_base_score + 500 THEN
    RAISE EXCEPTION 'impossible score';
  END IF;
  IF p_jumps > floor(p_play_time_ms / 80.0)::integer + 300 THEN
